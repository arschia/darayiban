...
| Supabase Postgres | تراکنش‌ها، بودجه‌ها، تعهدات، دارایی‌ها، پروفایل و توکن‌های اتوماسیون |
| Row Level Security | محدود کردن هر query به مالک همان رکورد |
| Edge Function `ingest-sms` | اعتبارسنجی توکن، تحلیل پیامک، حذف تکراری‌ها و ثبت تراکنش |
...
sequenceDiagram
  ...
  API->>API: تشخیص نوع، مبلغ، بانک و تاریخ
  API->>DB: ثبت تراکنش و ingest event
  API-->>IOS: نتیجه JSON
```
